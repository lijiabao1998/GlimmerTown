...
assert(/const CACHE=CACHE_PREFIX\+'v3';/.test(swSrc), 'sw.js CACHE 應由專屬前綴組成 v3');
...
async function runT268() {
  ...
  function makeSwHarness268(source) {
    ...
    const ops = { opened: [], addAll: [], deleted: [], puts: [], putFailures: 0, skipWaiting: 0, claim: 0, order: [] };
    ...
    const cacheStorage = {
      async open(name) { ops.opened.push(name); return cacheFor(name); },
  ...
  for (const name of ['gv-v1', 'gv-v2', 'glimmerville-shell-v2', 'gv-other-app', 'shared-cache']) h268.seed(name);
  await h268.fireLife('install');
  assert(h268.ops.opened[0] === 'glimmerville-shell-v3',
    'T268 install 應開啟專屬 glimmerville-shell-v3 cache');
  ...
  assert(JSON.stringify(h268.ops.deleted.slice().sort()) ===
    JSON.stringify(['glimmerville-shell-v2', 'gv-v1', 'gv-v2'].sort()),
    'T268 activate 應只刪本專屬舊版與精確 legacy gv-v1/v2');
  const keys268 = await h268.cacheStorage.keys();
  assert(keys268.includes('glimmerville-shell-v3') && keys268.includes('gv-other-app') && keys268.includes('shared-cache'),
  ...
  await h268.cacheStorage.delete('glimmerville-shell-v3');
  h268.setFetch(async () => { throw new TypeError('offline'); });
  result268 = await h268.fireFetch({ method: 'GET', url: base268, mode: 'navigate' });
...
runT268().then(() => {
  ...
  console.error('FAIL: T268 非預期例外', err && err.stack ? err.stack : err);
  process.exit(1);
});
