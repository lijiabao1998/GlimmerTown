...
const SVK = new Set([19, 25, 31, 32, 53, 54, 55, 56, 57, 58, 59, 61, 62, 63, 64, 65, 66, 68, 82, 83, 87, 90]); // T296：faithCenter 信仰中心單變體 // T265：wasteIncinerator；T257：nuclear/hydro/fireHQ 亦屬單變體；T228/T233/T234/T241/T254：bigFarm/bigCemetery/grandStation/sportsComplex/foodPlant；T230/T232：ranch/parking 移出
let tampered = 0;
for (const rec of d1.bl) if (SVK.has(rec[1])) { rec[3] = 2; tampered++; }
assert(tampered === 22, '存檔 bl 應含單變體建築（實得 ' + tampered + '）');
...
assert(/const CACHE=CACHE_PREFIX\+'v31';/.test(swSrc), 'sw.js CACHE 應由專屬前綴組成 v31');
...
async function runPwaTests() {
  ...
  assert(h268.ops.opened[0] === 'glimmerville-shell-v31',
    'T268-T270 install 應開啟目前專屬 glimmerville-shell-v31 cache');
  ...
  assert(keys268.includes('glimmerville-shell-v31') && keys268.includes('gv-other-app') && keys268.includes('shared-cache'),
  ...
  await h268.cacheStorage.delete('glimmerville-shell-v31');
  h268.setFetch(async () => { throw new TypeError('offline'); });
  result268 = await h268.fireFetch({ method: 'GET', url: base268, mode: 'navigate' });
  ...
  assert(h269.ops.opened[0] === 'glimmerville-shell-v31' &&
  ...
  assert(!keys269.includes('glimmerville-shell-v3') && !keys269.includes('glimmerville-shell-v4') &&
    keys269.includes('glimmerville-shell-v31') &&
  ...
  const cache269 = await h269.cacheStorage.open('glimmerville-shell-v31');
  ...
  await h269.cacheStorage.delete('glimmerville-shell-v31');
  result269 = await h269.fireFetch({
  ...
  await h271Miss.cacheStorage.delete('glimmerville-shell-v31');
...
runPwaTests().then(() => {
  ...
  // ===== T324 AI 市長適應性建設：62 種從不蓋 → 需求驅動補齊；財政閘門保證不拖垮窮城 =====
  {
    ...
    assert(window.GV.stats().pop === 3535, 'T324/T335b seed301 400天 pop 應恆為 3535（wants 輪轉+旅宿費率校準後重釘），實得 ' + window.GV.stats().pop);
