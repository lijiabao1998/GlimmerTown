// T32 警察局系統最小 DOM mock 測試
const fs = require('fs');
const path = require('path');

// ---- 最小 DOM / BOM mock（複製自 test_t31.js）----
const elMap = new Map();
function makeEl(tag, id) {
  const listeners = {};
  const children = [];
  const style = {};
  const dataset = {};
  const el = {
    tagName: tag,
    id: id || '',
    className: '',
    classList: { add() {}, remove() {}, contains() { return false; } },
    style,
    dataset,
    textContent: '',
    innerHTML: '',
    width: 100,
    height: 100,
    addEventListener(ev, fn) { listeners[ev] = listeners[ev] || []; listeners[ev].push(fn); },
    removeEventListener() {},
    dispatchEvent(ev) { (listeners[ev.type] || []).forEach(fn => fn(ev)); return true; },
    click() { this.dispatchEvent({ type: 'click' }); },
    appendChild(c) { children.push(c); return c; },
    insertBefore(c) { children.unshift(c); return c; },
    removeChild(c) { const i = children.indexOf(c); if (i >= 0) children.splice(i, 1); return c; },
    remove() { if (el.parentNode && el.parentNode.removeChild) el.parentNode.removeChild(el); },
    querySelector() { return null; },
    querySelectorAll() { return []; },
    getContext(type) {
      if (type === '2d') {
        return {
          save() {}, restore() {}, translate() {}, scale() {}, rotate() {},
          beginPath() {}, closePath() {}, moveTo() {}, lineTo() {}, rect() {}, fillRect() {}, strokeRect() {},
          arc() {}, arcTo() {}, ellipse() {}, quadraticCurveTo() {}, bezierCurveTo() {},
          fill() {}, stroke() {}, clip() {},
          fillText() {}, strokeText() {}, measureText() { return { width: 10 }; },
          drawImage() {}, putImageData() {}, getImageData() { return { data: new Uint8ClampedArray(4) }; },
          createLinearGradient() { return { addColorStop() {} }; },
          createRadialGradient() { return { addColorStop() {} }; },
          createPattern() { return {}; },
          setTransform() {}, resetTransform() {}, clearRect() {}, setLineDash() {},
          globalAlpha: 1, fillStyle: '', strokeStyle: '', lineWidth: 1, font: '', textAlign: '', textBaseline: '',
          canvas: el
        };
      }
      return null;
    },
    children,
    _listeners: listeners
  };
  if (id) elMap.set(id, el);
  return el;
}

const ids = ['game','hud','money','day','pop','jobs','happy','rci','bStats','bHelp','bUndo','bSpeed','bSound','bSave','bNew','hint','hintTxt','hintX','tools','toasts','dragcost','mini','zoomer','zin','zout','info','infoX','infoBody','start','logo','bContinue','bNewGame'];
ids.forEach(id => makeEl('div', id));
makeEl('canvas', 'game');
makeEl('canvas', 'logo');
makeEl('canvas', 'mini');

const document = {
  querySelector(s) {
    if (s.startsWith('#')) return elMap.get(s.slice(1)) || makeEl('div');
    if (s.startsWith('.')) return makeEl('div');
    return makeEl(s);
  },
  querySelectorAll(s) {
    if (s === '.tool') return [];
    return [];
  },
  createElement(tag) { return makeEl(tag); },
  title: '',
  addEventListener() {},
  removeEventListener() {},
  hidden: false,
  visibilityState: 'visible',
  body: makeEl('body')
};

const store = {};
const localStorage = {
  getItem(k) { return store[k] === undefined ? null : store[k]; },
  setItem(k, v) { store[k] = String(v); },
  removeItem(k) { delete store[k]; }
};

const navigator = {
  clipboard: null,
  serviceWorker: { register: () => Promise.resolve() }
};

const location = { protocol: 'file:' };

const window = {
  devicePixelRatio: 1,
  addEventListener() {},
  removeEventListener() {},
  AudioContext: function() { this.state = 'suspended'; this.createGain = () => ({ gain: { value: 0 }, connect() {} }); this.createOscillator = () => ({ connect() {}, start() {}, stop() {}, frequency: { value: 0 } }); this.destination = {}; this.suspend = () => {}; this.resume = () => {}; },
  webkitAudioContext: function() { return new window.AudioContext(); },
  requestAnimationFrame() {},
  GV: undefined,
  document,
  localStorage,
  navigator,
  location
};

global.window = window;
global.document = document;
global.navigator = navigator;
global.localStorage = localStorage;
global.location = location;
global.requestAnimationFrame = window.requestAnimationFrame;
global.AudioContext = window.AudioContext;
global.webkitAudioContext = window.webkitAudioContext;

// ---- 載入 index.html 中的 script ----
const html = fs.readFileSync(path.join(__dirname, 'index.html'), 'utf8');
let js = '';
let i = 0;
while (true) {
  const s = html.indexOf('<script>', i);
  if (s === -1) break;
  const e = html.indexOf('</script>', s);
  if (e === -1) break;
  js += html.slice(s + 8, e) + '\n';
  i = e + 9;
}
eval(js);

// ---- 測試輔助 ----
function assert(cond, msg) {
  if (!cond) { console.error('FAIL:', msg); process.exit(1); }
  console.log('PASS:', msg);
}

const N = window.GV.N();
function idx(x, y) { return y * N + x; }
function tile(x, y) { return window.GV.tile(x, y); }
function place(t, x, y) { return window.GV.place(t, x, y); }
function step(n) { return window.GV.step(n); }

// ---- 場景：道路 + 電廠 + 兩組住宅/商業 ----
window.GV.newWorld();
window.GV.weather(0);

// 找一個離邊緣夠遠的陸地中心
let cx = Math.floor(N / 2), cy = Math.floor(N / 2);
outer: for (let y = 10; y < N - 20; y++) {
  for (let x = 10; x < N - 20; x++) {
    let ok = true;
    for (let dy = -5; dy <= 15 && ok; dy++) {
      for (let dx = -8; dx <= 8 && ok; dx++) {
        const t = tile(x + dx, y + dy);
        if (!t || (t.t !== 1 && t.t !== 2)) ok = false;
      }
    }
    if (ok) { cx = x; cy = y; break outer; }
  }
}
console.log('build center:', cx, cy);

// 橫向主幹道
for (let x = cx - 12; x <= cx + 12; x++) place('road', x, cy);
// 縱向道路到 B 區
for (let y = cy; y <= cy + 13; y++) place('road', cx, y);
// B 區橫向道路
for (let x = cx - 3; x <= cx + 3; x++) {
  place('road', x, cy + 11);
  place('road', x, cy + 13);
}
// 電廠（A/B 各一，確保道路通電）
place('plant', cx - 10, cy - 1);
place('plant', cx - 2, cy + 6);
place('plant', cx + 2, cy + 12);

// A 區：警察局半徑10內（受保護）
for (let x = cx - 3; x <= cx + 3; x++) {
  place('zr', x, cy - 1);
  place('zr', x, cy + 1);
}
place('police', cx, cy - 3);

// B 區：警察局半徑10外（未受保護），距離 >10
for (let x = cx - 3; x <= cx + 3; x++) {
  place('zr', x, cy + 11);
  place('zr', x, cy + 13);
  place('zc', x, cy + 12);
}

// 先跑一段時間讓建築生長
step(30);
let protectedCrime = 0;
let unprotectedCrime = 0;
for (let x = cx - 3; x <= cx + 3; x++) {
  for (let y of [cy - 1, cy + 1]) {
    const b = tile(x, y).bld;
    if (b && b.crime) protectedCrime++;
  }
}
for (let x = cx - 3; x <= cx + 3; x++) {
  for (let y of [cy + 11, cy + 13, cy + 12]) {
    const b = tile(x, y).bld;
    if (b && b.crime) unprotectedCrime++;
  }
}
console.log('after 30 steps: protectedCrime', protectedCrime, 'unprotectedCrime', unprotectedCrime);

// 由於犯罪率很低（0.001/天），再跑一段時間增加樣本
step(100);
protectedCrime = 0;
unprotectedCrime = 0;
for (let x = cx - 3; x <= cx + 3; x++) {
  for (let y of [cy - 1, cy + 1]) {
    const b = tile(x, y).bld;
    if (b && b.crime) protectedCrime++;
  }
}
let unprotectedBuildings = 0;
for (let x = cx - 3; x <= cx + 3; x++) {
  for (let y of [cy + 11, cy + 13, cy + 12]) {
    const b = tile(x, y).bld;
    if (b) {
      unprotectedBuildings++;
      if (b.crime) unprotectedCrime++;
    }
  }
}
console.log('after 130 steps: protectedCrime', protectedCrime, 'unprotectedCrime', unprotectedCrime, 'unprotectedBuildings', unprotectedBuildings);

assert(protectedCrime === 0, '警察局半徑10內不應發生犯罪');

// 強制在未受保護區域觸發犯罪，測試犯罪機制與幸福影響
let crimeTarget = null;
outer: for (let x = cx - 3; x <= cx + 3; x++) {
  for (let y of [cy + 11, cy + 13, cy + 12]) {
    const t = tile(x, y);
    if (t.bld && t.bld.k <= 3) { crimeTarget = { x, y }; break outer; }
  }
}
assert(crimeTarget, '應找到可觸發犯罪的未受保護建築');
// 找一個犯罪目標旁有建築的鄰居，測試幸福影響
let neighbor = null;
for (let dy = -2; dy <= 2 && !neighbor; dy++) {
  for (let dx = -2; dx <= 2 && !neighbor; dx++) {
    if (dx === 0 && dy === 0) continue;
    const nx = crimeTarget.x + dx, ny = crimeTarget.y + dy;
    const t = tile(nx, ny);
    if (t.bld && t.bld.k === 1) { neighbor = { x: nx, y: ny }; }
  }
}
const hBeforeNeighbor = neighbor && tile(neighbor.x, neighbor.y).bld ? tile(neighbor.x, neighbor.y).bld.h : null;
window.GV.igniteCrime(crimeTarget.x, crimeTarget.y);
assert(tile(crimeTarget.x, crimeTarget.y).bld && tile(crimeTarget.x, crimeTarget.y).bld.crime === 1, '強制觸發犯罪後 crime=1');
step(1); // 讓幸福公式重新計算
const hAfterNeighbor = neighbor && tile(neighbor.x, neighbor.y).bld ? tile(neighbor.x, neighbor.y).bld.h : null;
console.log('neighbor happiness before/after crime:', hBeforeNeighbor, hAfterNeighbor);
if (hBeforeNeighbor !== null && hAfterNeighbor !== null) {
  assert(hAfterNeighbor < hBeforeNeighbor, '鄰近建築幸福應因犯罪下降');
}

// 測試處理犯罪：清除剛才強制觸發的犯罪
window.GV.clearCrime(crimeTarget.x, crimeTarget.y);
assert(tile(crimeTarget.x, crimeTarget.y).bld.crime === 0, '處理犯罪後 crime 應為 0');

// 測試經濟面板 upPol
const fin = window.GV.stats();
console.log('stats:', fin);
// fin 沒有 upPol 鉤子，但可確認警察局計入維護費：建築數增加即可

// 測試存檔/讀檔：保存後讀取，確認警察局與犯罪標記保留
const beforeSave = JSON.parse(JSON.stringify(tile(cx, cy - 3)));
window.GV.save();
// 人為製造犯罪後存檔
window.GV.igniteCrime(crimeTarget.x, crimeTarget.y);
assert(tile(crimeTarget.x, crimeTarget.y).bld.crime === 1, '存檔前犯罪標記應為 1');
window.GV.save();
// 重新載入
window.GV.newWorld();
window.GV.load();
const afterLoad = tile(cx, cy - 3);
assert(afterLoad.bld && afterLoad.bld.k === 11, '讀檔後警察局應保留');
const victimAfter = tile(crimeTarget.x, crimeTarget.y);
assert(victimAfter.bld && victimAfter.bld.crime === 1, '讀檔後犯罪標記應保留');

console.log('\nT32 DOM mock 測試全部通過');
process.exit(0);
