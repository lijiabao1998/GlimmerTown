...
# constrained to a small-ish set of common Chinese characters
raw_samples = {
    "CITY": [
    ...
    "BLD": [
        ...
        "紅磚小屋","青石別墅","紫晶豪宅","銀河中心","星辰大樓","月光劇院","朝陽市場",
        "晚霞餐廳","晨曦咖啡","暮光書店","彩虹超市","雲端旅館","風車磨坊","燈塔郵局",
        "港灣倉庫","楓葉學校","柳溪醫院","竹林車站","松林旅社","水晶劇場","寶石銀行",
        "琥珀酒館","翡翠茶樓","瑪瑙工坊","琉璃溫泉","珍珠飯店","鑽石賭場","黃金交易所",
        "白銀當鋪","鐵砧工坊","銅鑼鐘樓","紡織工房","釀酒工廠","麵包工坊","糖果小屋","玩具商店"
    ],
    "THO": [
        "公園真美","工廠好吵","稅收好高","生活便利","交通便利","空氣新鮮","夜景迷人",
        "房價太貴","工作很多","上學方便","醫療完善","購物方便","娛樂太少","綠地很多",
        "噪音嚴重","電力穩定","網路很快","鄰居友善","街道乾淨","治安良好","失業擔心",
        "商業熱鬧","工業影響","公園放鬆","學校優秀","醫院救命","超市齊全","餐廳美食",
        "房價合理","通勤太長","夜生活好","白天太吵","晚上安靜","夏天涼爽","冬天溫暖",
        "四季如春","風景如畫","生活舒適","交通順暢","環境優美","設施齊全","居民滿意"
    ],
    "NEW": [
        ...
        "環保政策上路","垃圾分類成效顯著","自行車系統擴充","免費網路覆蓋全市","智慧城市計畫啟動"
    ]
}
...
# build vocab: keep top K most frequent chars + mandatory prefixes + UNK
MAX_VOCAB = 120
freq = {}
for s in samples:
    for ch in s:
...
UNK = "·"
mandatory = set("[]CTYBLDHOENW\n") | {UNK}
_top = sorted([c for c in freq if c not in mandatory], key=lambda c: -freq[c])[:MAX_VOCAB - len(mandatory)]
chars = sorted(mandatory | set(_top))
char_to_idx = {c: i for i, c in enumerate(chars)}
idx_to_char = {i: c for c, i in char_to_idx.items()}
vocab_size = len(chars)
print(f"vocab_size={vocab_size}, samples={len(samples)}")

# replace out-of-vocab chars in samples with UNK (rare after pruning)
fallback_idx = char_to_idx[UNK]
for i, s in enumerate(samples):
    samples[i] = "".join(c if c in char_to_idx else UNK for c in s)
