...
const evalJs = async (expr, ms = 5000) => {
  ...
  if (!r) throw new Error('eval timeout');
  if (r.exceptionDetails) throw new Error(r.exceptionDetails.exception?.description?.slice(0, 150));
  return r.result?.result?.value;
};
await send('Runtime.enable');
await evalJs(`location.href='http://localhost:8129/index.html'`);
let ready = false;
for (let i = 0; i < 25; i++) {
  await sleep(1000);
  try { if (await evalJs(`typeof GV!=='undefined'&&!!document.querySelector('#bNewGame')`) === true) { ready = true; break; } } catch (e) {}
}
