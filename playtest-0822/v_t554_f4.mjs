...
const planStr = await evalJs(`JSON.stringify((()=>{const ok=[];for(let x=2;x<70;x++){if(GV.place('road',x,6))ok.push(x);}return {n:ok.length,cands:ok.slice(0,10).map(x=>[x,7])};})())`);
...
for (const [gx, gy] of p0.cands) {
  ...
  await c.send('Input.dispatchMouseEvent', { type: 'mousePressed', x: ctr[0], y: ctr[1], button: 'left', clickCount: 1 });
  await sleep(80);
  await c.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: ctr[0], y: ctr[1], button: 'left', clickCount: 1 });
  await sleep(400);
