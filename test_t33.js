// T33 醫院系統最小 DOM mock 測試
const fs = require('fs');
const path = require('path');

// ---- 最小 DOM / BOM mock（複製自 test_t32.js）----
const elMap = new Map();
...
// ---- 場景：道路 + 電廠 + 兩組住宅 ----
// 使用固定種子讓測試可重現；可透過 GT_SEED 環境變數覆蓋
...
const hospitalPlaced = place('hospital', cx - 2, cy + 1);
assert(hospitalPlaced, '醫院應成功建造');

// B 區：醫院半徑12外（未受保護），距離 >12；混合分區刺激 RCI
for (let x = cx - 3; x <= cx + 3; x++) {
  place('zr', x, cy + 10);
  place('zc', x, cy + 12);
  place('zr', x, cy + 14);
...
assert(protectedTarget, '應找到醫院覆蓋區內的住宅');
assert(unprotectedTarget, '應找到醫院覆蓋區外的住宅');
...
// 跑一天，覆蓋區應康復，未覆蓋區仍生病
step(1);
assert(tile(protectedTarget.x, protectedTarget.y).bld.sick === 0, '醫院覆蓋內住宅次日應康復');
assert(tile(unprotectedTarget.x, unprotectedTarget.y).bld.sick === 1, '醫院覆蓋外住宅應保持生病');
...
const loadedHospital = tile(cx - 2, cy + 1);
assert(loadedHospital.bld && loadedHospital.bld.k === 12, '讀檔後醫院應保留');

// 測試經濟面板 fin.upH
const fin = window.GV.stats();
console.log('stats:', fin);
// 醫院計入維護費主要透過 fin.upH，測試腳本透過 stats() 無法直接取到，但存檔/建築保留已驗證
