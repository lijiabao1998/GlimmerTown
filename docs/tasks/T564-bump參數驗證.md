...
**工具鏈 61 → 63 例**　**master `17b000c`**

紅源（拔驗證 → 拒絕例紅）咬中、還原綠。過程自捕：測試造假倉庫首版用 write_text——
Windows 文字模式換行轉 CRLF、被 bump 自己的鐵律 20 斷言咬（環境陷阱 §5 的測試版），
改 write_bytes。事故本身的兩個僥倖（紅源還原自癒 index、git checkout 救 sw）如實入帳：
若當時沒有紅源任務在跑，壞版本會一路帶進收束。
