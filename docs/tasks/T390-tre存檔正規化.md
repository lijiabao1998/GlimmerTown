...
狀態:**已完成(v11.17,bay/kimi)。PASS 3462→3474;救援歧義兩啟發式量測記卡(最左 467/合併優先 698)。**
...
### 救援歧義的量測與裁決(本卡最有價值的一段)
seed1 病檔(67 v10/5251 字元):最左解與原檔差 **467/5184 格**;合併優先變體 **698** 更差
——"1"+"0" 相鄰對(樹種1+空草 vs 真 v10)在檔內**資訊理論上不可分辨**,「歧義窗有界」
的出卡預期撤回。裁決:保留最左解;驗收改為數學正確性(再編碼恆等=病檔的合法等價解讀+
全錨通過+長度歸正),差異量純診斷回報。對照組=fallback 全滅,467 格樹種局部互換是
顯著較優的玩家結果。

### 過程二修(誠實記錄)
1. repairTre390 首版誤插 load() 內層(vlen 是 load 區域 const)→ ReferenceError→搬模組層。
2. T371b ARCH 地圖錨點含舊 encode 字面,被本卡改動咬紅→同卡更新錨(守衛網互咬=設計如此,
   T371b 的存在理由再驗證一次)。

### 閘門
PASS 3462→3474/0 FAIL;verify ALL GREEN 11.17;五組守衛+觸發行釘;破壞性三案紅源驗明
(p1 雙咬:長度守衛+ARCH 錨)。
