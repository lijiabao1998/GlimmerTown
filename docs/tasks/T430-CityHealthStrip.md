...
狀態：**已合併部署 v11.66**（業主明示「完整出卡并施工完成，你出卡，你施工，你合并dsk」）
> **T433 盤點發現的未修缺陷（本卡未退修，僅留痕）**：`index.html:12808`（**R02 更正**：第一版寫 12769，那是夜班盤點報告給的行號、我沒複驗就寫進了三份文件；12769 實為 T127 的 `trC++` 那一行。實測 `grep -n "b.k>3&&b.k!==127" index.html` → 12808） 的
> `if(!b||(b.k>3&&b.k!==127))continue;` 讓 `b.pw`／`b.wa` 的每日重算只跑 k=1/2/3＋k=127，
> 且全檔 `b.wa=` 只有一處寫入端 ⇒ **所有 k≥4 的建築（服務／公共／農牧／地標）
> ⚡格永遠 good（不隨 T432 電網分區變動）、💧格永遠 bad**。本卡 §6.4 自訂的
> 「真瀏覽器供電／斷電兩態實拍」從未交付，作者唯一的樣本（k=63 溫室）恰好就是這類假資料。
> 處置需業主選（補 k≥4 真實重算＝動 tick＝必破兩釘 vs 非 RCI 兩格降級成 na）。（T433 2026-08-14 校正：本行原本停在合併前狀態，與 docs/CHANGELOG.md 矛盾）
