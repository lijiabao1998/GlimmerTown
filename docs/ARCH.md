...
**現況（測繪基準）**：單檔 `index.html`，v11.42，約 20,138 行（實測檔案 20,147 行＝T417 四輪覆核退修閉合後 wc -l 重测；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T417**（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 4,157、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
