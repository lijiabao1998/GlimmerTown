...
**現況（測繪基準）**：單檔 `index.html`，v10.7，約 17,820 行（實測檔案 17,820 行＝T375 施工當時；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T375**（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 2,970+、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
...
> T375 已收掉原表的六列（車道標線／單行道箭頭／動態浪花／地鐵 overlay／廣場踏步／T361c 季節粒子，
> v10.7）。下表為剩餘四列，行號已依 T375 後現況校正；B／C／E 三族各自另卡（T376/T377/T378）。
| # | 位置 | 症狀 |
|---|---|---|
| 1 | `fxParts` 的 `dep`（12043 `spawnDust` 起的七個 spawn 點）是世界 `x+y` | 位置對、深度排序鍵錯（→ T377） |
| 2 | 載具 A/B 挑圖與 `DIRSCR` 車燈向量（12005；用法 13378/13389） | rot=1/3 時南北↔東西互換，恰好選反（→ T376） |
| 3 | 12713 `horizonY=Math.max(0,Math.round(syOf(0,0)+6*z))` | rot=1/3 地平線掉到菱形腰線、rot=2 掉到底點，T256 懸浮感復發（→ T378） |
| 4 | 12749 懸崖 `x===N-1\|\|y===N-1` | rot=2 時崖壁畫在後緣、近端無崖（→ T378） |
