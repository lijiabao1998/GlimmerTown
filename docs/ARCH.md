...
**現況（測繪基準）**：單檔 `index.html`，v10.5，約 17,667 行（實測檔案 17,701 行＝T371b 測繪當時；T372 後為 17796 行。**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；T372 後為 17796 行，行號以本次測繪為準——**行號會漂移，定位一律以 grep 錨點為準**）；卡號已到 **T370**（CHANGELOG 379 條）；測試 `test_fixde.js` 4,574 行、實測 PASS 2,875、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
