...
v3.8（T136）季節純由 `day` 導出，存檔格式零變更。v3.8（T137）追加一個可選欄位：`plague`＝瘟疫狀態（`{cells,days}` 最小結構，仿 `riot` 慣例，`load()` 於 `riot` 還原之後同批還原，缺欄位視為無瘟疫）；同卡新增的 `drought`/`blizzard`/`leakDays` 皆運行時狀態，不入存檔本體。v3.8（T138/T139）建築型別擴充（k=35-48）存檔格式零變更——`bl` 元組既有 `sz`/`ref` 機制天然支援新多格建築，`load()` 依 `MSZ` 表反查即可正確重建，未新增任何欄位。
