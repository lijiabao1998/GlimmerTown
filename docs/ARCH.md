...
**現況（測繪基準）**：單檔 `index.html`，v11.99，約 22,160 行（實測檔案 22,160 行＝T461 城市科學財政條後 Python 數；測試 `test_fixde.js`）。零執行期相依、無框架無 CDN、全部美術程序化生成。
...
| 季節與 360 天年 / `foodPriceOf` | 1090 | `const season=()=>{const doy=(day-1)%360;` |
| T343 科技樹（36 節點／研究狀態／集中 `tq` 效果） | 1246 | `const TECH343=[` |
| §2 繪圖基元：`cv` / `dia` / `railTrack` / `isoBox` / `outlineSprite` | 1553 | `function cv(w,h){const c=document.createElement('canvas')` |
| 紋理流 `spriteTexRand` ＋ `plate()` | 1940 | `const SPRITE_TEX_SEED=0x54455832;` |
| `HERO_PIX` 手繪點陣表（53 鍵） | 1965 | `const HERO_PIX={` |
| **`buildSprites()` 全體** | 2082 | `__savedR=R;R=mulberry32(1);` |
| ├ 地形／道路／覆蓋層 | 2318 | `/* ---------- 草地 4 變化 ---------- */` |
| ├ `PARTS` / `DRAFTS` / `mkBld` | 2153 | `const PARTS=` |
| ├ 服務建築與變體 | 2482 | `/* ---------- 公園 3 變化 ---------- */` |
|　├ 體育場| 2906 | `/* ---------- 體育場（T22；首個 2×2 多格建築；136×150、錨點 68/` |
|　├ 墓園| 3390 | `/* ---------- 墓園（T38；72×112 小教堂＋墓碑群＋柏樹＋夜間微弱燭光；3 ` |
|　├ T382：鐘樓細化| 3855 | `/* T382：鐘樓細化（零亂數）——門、台階、壁燈、石紋 */` |
|　├ T382：度假酒店細化| 4337 | `/* T382：度假酒店細化（零亂數）——陽台欄、大堂門廊、池邊躺椅、遮陽傘、水線、翼門、夜池邊` |
|　├ T382：社區菜園細化| 4829 | `/* T382：社區菜園細化（零亂數）——澆水壺、圍欄、小徑、補夜層（退修：澆水壺下移，避免 v` |
| ├ 多格地標長列 | 5008 | `T355 版面重排` |
| ├ FIX-B 尾端區起點 | 5374 | `/* ===== FIX-B 亂數流對齊：以下 T29/T30/T35/T36` |
|　├ T382：綠蔭停車 v2 細化| 5759 | `/* T382：綠蔭停車 v2 細化（零亂數）——充電樁、路緣、地磚 */` |
|　├ 公園新變體 3 種| 6228 | `/* ---------- 公園新變體 3 種（噴泉廣場／玫瑰園／遊樂場） ----------` |
|　├ 立面件：沿左右牆挑 dx 段落| 6720 | `// ---- 立面件：沿左右牆挑 dx 段落，y 落在該 dx 列牆面安全範圍內（yF-h+p` |
|　├ 街道設施小件| 7183 | `// ---- 街道設施小件（沿用 SPR.rdec/SPR.busStop 同款 64×40／` |
|　├ T150 變體大擴充：住宅/商業/工業每級 8→12 變體| 7440 | `/* ===== T150 變體大擴充：住宅/商業/工業每級 8→12 變體（k=1/2/3 各` |
|　├ T382：hero 後寫加蓋——救護車、入口雨棚、紅十字角燈 */| 7940 | `/* T382：hero 後寫加蓋——救護車、入口雨棚、紅十字角燈 */` |
|　├ T382：hero 後寫加蓋——候診椅、門廊、綠十字角燈 */| 8436 | `/* T382：hero 後寫加蓋——候診椅、門廊、綠十字角燈 */` |
|　├ T380：美術改畫在田外上緣自由區| 8927 | `/* T380：美術改畫在田外上緣自由區（y≤105；田心 cy=128 hh=20 不蓋），保` |
|　├ T278 RCI 屋頂雜項加蓋| 9391 | `/* ===== T278 RCI 屋頂雜項加蓋（TheoTown 級屋頂密度）：snowCap` |
| ├ 加蓋層 pass 群（T277→T345→**T382**） | 9450 | `const stampRoof=(key)=>{` / `/* ===== T382 全局美術極細化加蓋 pass` |
| └ T364 縮放管線與收尾 | 9882 | `const mkIndustry364=(kind)=>{` |
|　├ T425 統一底部貼合 pass| 10364 | `/* ===== T425 統一底部貼合 pass（buildSprites 絕對尾端）：T29` |
| §3 世界生成 `genWorld` / `newWorld` | 10651 | `FIX-K：天氣/災害殘留狀態種子化重置` |
| §4 `canPlace` / `placeCost` / `doPlace` | 10726 | `function canPlace(toolId,x,y){` |
|　├ doPlace| 11018 | `function doPlace(toolId,x,y,silent){` |
| `computePower` | 11730 | `function computePower(){` |
| `COVR` / `COV` / `covFieldOfK` / `stampCov` | 11944 | `const covFieldOfKBase=covFieldOfK;` |
| POL / NOISE 場 | 11965 | `function stampPolSrc(x,y,k,sign){` |
| LAND 場 | 12056 | `function landStaticAt(x,y){` |
| EDU / `rebuildCov` / `judgeWealth` | 12104 | `function rebuildCov(){` |
| `computeWater` / `resilience364At` | 12195 | `function computeWater(){` |
| §5 `aiStep()` AI 市長 | 12345 | `let acts=0;const MAXA=14,RESERVE=poor?50:350;` |
| `buildTickIndex()` | 12862 | `function buildTickIndex(){` |
| **`tick()` 全體** | 12875 | `rebuildNoise(tickBld); // T325` |
| ├ 第一經濟迴圈（計數／幸福） | 12970 | `for(const i88 of tickBld){` |
| ├ 產業鏈 | 13205 | `/* ===== T364b A 深加工鏈 BEGIN =====` |
| ├ 災害段 | 13262 | `// T69/T70 災害（可關）` |
| ├ 生長／升級／合併／火災 | 13430 | `// 生長：收集候選` |
| ├ **第二經濟迴圈（稅收守衛鏈）** | 13716 | `const civicMul=chN>0?1.03:1;` |
| └ 維護費／評分／`aiStep` 呼叫 | 13867 | `if(diff!==3)money+=income-upkeep;` |
|　├ updCars| 14304 | `function updCars(dt){` |
| §6 載具與煙／`computeCommute` | 15019 | `function computeCommute(){` |
| §7 繪製起點：`DIRSCR` / 雨雪 | 14589 | `const DIRSCR=[[4,-2],[4,2],[-4,2],[-4,-2]];` |
| 粒子池 `fxParts` | 14619 | `/* ---------- T159 粒子特效豐富化` |
| `daylight()` | 14715 | `function daylight(){` |
| `streetHash` | 15104 | `function streetHash(x,y,salt){` |
| **T367 旋轉變換層** | 15111 | `/* ===== T367 視角四向旋轉：view-space 變換層` |
| 地面快取全域 | 15200 | `let groundDirty=true,groundCache=null,` |
| **`draw()` 全體** | 15444 | `function draw(dt){` |
| ├ 地面層與 groundCache 重烘 | 15587 | `// ---- 地面層 ----（T96：離屏快取` |
| ├ `objs` 收集與深度排序 | 16006 | `// ---- 物件層（依深度排序） ----` |
| ├ 地格分支（建築十餘層加疊） | 16444 | `const t=o.t;` |
|　├ T403b/c 農事與牧場動態| 16927 | `/* ===== T403b/c 農事與牧場動態（T368a 純視覺路線：零狀態、零陣列，一切由` |
| ├ 天氣後製與晝夜 multiply | 17088 | `// ---- 天氣色調（雨天壓暗` |
| └ 夜燈層 `nightSprites` | 17283 | `// ---- 夜間燈光（T212` |
|　├ drawGroundMemory428| 17778 | `function drawGroundMemory428(g,x,y,sx,sy,z){` |
| `drawCursor` / `drawHoverLabel` / `toTile` | 18150 | `function drawHoverLabel(sxOf,syOf,z){` |
| undo 群組 helper | 18221 | `const openUndo=()=>{undoGroup={snaps:[],seen:{},spent:0};};` |
| `commitRect` 批次施作 | 18423 | `const hiBld=(x,y)=>{const b=T(idx(x,y)).bld;` |
| `UP_MAX` / `upCost` / `upgradeBld` | 18453 | `const UP_MAX={9:15,6:10,7:10,11:10,12:10` |
| `inspect()` 建築檢視面板 | 18479 | `function svcStatTable(x,y,b){` |
|　├ MOODS| 18910 | `const MOODS={` |
| §10 UI 起點：`buildToolbar` | 19177 | `function buildToolbar(){` |
| `drawMini` 小地圖 | 19286 | `const MINI_VIEW_NAME=` |
| `statTab` / `dataTable` / `serviceRefRows` | 19361 | `T319 標準化數據表格產生器` |
| `toast` / `updHud` / HUD 三鈕 | 19428 | `T311 提示訊息不刷屏` |
| 存檔槽面板／分享碼 | 19523 | `function showSlots(){` |
| 地圖／場景編輯器 | 19632 | `function showEditor(){` |
| 歷史曲線／城市顧問／因果追溯 | 19718 | `function cityAdvisor(){` |
| `showStats()` 統計面板 | 20174 | `function showStats()` |
| 通知中心／成就 | 20565 | `T114：通知中心——HUD 鈴鐺按鈕` |
| `chipPanel()` 六晶片明細 | 20641 | `function chipPanel(which){` |
| `showHelp()` 五分頁指南 | 20843 | `const tabs=['🚦 上手流程'` |
| `undo()` / 快捷鍵 / 新手提示 | 21074 | `function checkHints(plants,roads){` |
| §11 存檔：槽位／RLE／`save` | 21092 | `function slotKey(n){return SAVEKEY+'.s'+n;}` |
| `MSZ` 多格尺寸反查表 | 21213 | `const MSZ={19:4,20:2,22:2,23:2,24:2,25:2` |
| `load()` | 16901-17037 | `const vlen=(dd)=>{const dn=+(dd&&dd.n)||72;` |
| 自動存檔 | 21398 | `setInterval(()=>{if(tiles)save();},25000);` |
| 主迴圈 `advance` / `frame` | 21411 | `while(simAcc>=DAYLEN&&steps<8){` |
| `begin` / `toMainMenu` / 開始畫面注入 | 21553 | `function toMainMenu(){` |
| `window.GV` 除錯鉤子（194 鍵） | 21648 | `window.GV={` |
