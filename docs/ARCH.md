...
**現況（測繪基準）**：單檔 `index.html`，v11.50，約 20,508 行（實測檔案 20,508 行＝T423 UI 現代化後 Python 數
；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T424**（TheoTown 風三鍵美術：消防局/學校/警察局 v0 hero 重繪為等距箱體＋材質肌理＋附屬物）（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 4,399、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
