...
**現況（測繪基準）**：單檔 `index.html`，v11.191，約 24959 行（實測檔案 24959 行＝LF換行計數；T581 軌道車輛動畫）。標準 `verify.py` 實得 **PASS 11,180／0 FAIL／exit0／ALL GREEN**，六哨兵不變，像素指紋 1,425 鍵（T581 新增 36 幀鍵，既有 1,389 鍵零變動）。T576／T577 已於 2026-09-08 併入 master `9f1b968`、T578 併入 `30b880e`、T580 併入 `69cb6da`，玩家端與 master 位元一致。**本行歷來兩度殘留「未合併／未部署」的過時敘述（T575 §4、T580 各更正一次）——`arch_map` 只驗表格錨點行號、T371b 只驗行數與版本字串，沒有守衛讀這段散文，已記入 docs/BACKLOG.md P3-1。** T574 的 AI 成長退化與效能風險仍在，見 T574 卡面與 BACKLOG P1。
