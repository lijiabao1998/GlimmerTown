...
**現況（測繪基準）**：單檔 `index.html`，v11.48，約 20,433 行（實測檔案 20,433 行＝T422 施工後 Python 數
；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T422**（遠景夜城 LOD：住宅暖、商業彩、工業冷、公共／地標白金，原位替換 T107 mini 夜燈且不增掃描／draw call）（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 4,372、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
