...
| LAND 場 | 12608 | `function landStaticAt(x,y){` |
| EDU / `rebuildCov` / `judgeWealth` | 12656 | `function rebuildCov(){` |
| `computeWater` / `resilience364At` | 12786 | `function computeWater(){` |
| §5 `aiStep()` AI 市長 | 12936 | `let acts=0;const MAXA=14,RESERVE=poor?50:350;` |
| `buildTickIndex()` | 13472 | `function buildTickIndex(){` |
| **`tick()` 全體** | 13485 | `rebuildNoise(tickBld); // T325` |
| ├ 第一經濟迴圈（計數／幸福） | 13580 | `for(const i88 of tickBld){` |
| ├ 產業鏈 | 13849 | `/* ===== T364b A 深加工鏈 BEGIN =====` |
| ├ 災害段 | 13906 | `// T69/T70 災害（可關）` |
| ├ 生長／升級／合併／火災 | 14074 | `// 生長：收集候選` |
| ├ **第二經濟迴圈（稅收守衛鏈）** | 14360 | `const civicMul=chN>0?1.03:1;` |
| └ 維護費／評分／`aiStep` 呼叫 | 14513 | `if(diff!==3)money+=income-upkeep;` |
|　├ updCars| 14955 | `function updCars(dt){` |
| §6 載具與煙／`computeCommute` | 15670 | `function computeCommute(){` |
| §7 繪製起點：`DIRSCR` / 雨雪 | 15240 | `const DIRSCR=[[4,-2],[4,2],[-4,2],[-4,-2]];` |
| 粒子池 `fxParts` | 15270 | `/* ---------- T159 粒子特效豐富化` |
| `daylight()` | 15366 | `function daylight(){` |
| `streetHash` | 15755 | `function streetHash(x,y,salt){` |
| **T367 旋轉變換層** | 15762 | `/* ===== T367 視角四向旋轉：view-space 變換層` |
| 地面快取全域 | 15851 | `let groundDirty=true,groundCache=null,` |
| **`draw()` 全體** | 16095 | `function draw(dt){` |
| ├ 地面層與 groundCache 重烘 | 16238 | `// ---- 地面層 ----（T96：離屏快取` |
| ├ `objs` 收集與深度排序 | 16657 | `// ---- 物件層（依深度排序） ----` |
| ├ 地格分支（建築十餘層加疊） | 17095 | `const t=o.t;` |
|　├ T403b/c 農事與牧場動態| 17578 | `/* ===== T403b/c 農事與牧場動態（T368a 純視覺路線：零狀態、零陣列，一切由` |
| ├ 天氣後製與晝夜 multiply | 17739 | `// ---- 天氣色調（雨天壓暗` |
| └ 夜燈層 `nightSprites` | 17934 | `// ---- 夜間燈光（T212` |
|　├ drawGroundMemory428| 18429 | `function drawGroundMemory428(g,x,y,sx,sy,z){` |
| `drawCursor` / `drawHoverLabel` / `toTile` | 18801 | `function drawHoverLabel(sxOf,syOf,z){` |
| undo 群組 helper | 18872 | `const openUndo=()=>{undoGroup={snaps:[],seen:{},spent:0};};` |
| `commitRect` 批次施作 | 19074 | `const hiBld=(x,y)=>{const b=T(idx(x,y)).bld;` |
| `UP_MAX` / `upCost` / `upgradeBld` | 19104 | `const UP_MAX={9:15,6:10,7:10,11:10,12:10` |
| `inspect()` 建築檢視面板 | 19130 | `function svcStatTable(x,y,b){` |
|　├ MOODS| 19561 | `const MOODS={` |
| §10 UI 起點：`buildToolbar` | 19828 | `function buildToolbar(){` |
| `drawMini` 小地圖 | 19937 | `const MINI_VIEW_NAME=` |
| `statTab` / `dataTable` / `serviceRefRows` | 20014 | `T319 標準化數據表格產生器` |
| `toast` / `updHud` / HUD 三鈕 | 20099 | `T311 提示訊息不刷屏` |
| 存檔槽面板／分享碼 | 20194 | `function showSlots(){` |
