...
**現況（測繪基準）**：單檔 `index.html`，v11.178，約 23500 行（實測檔案 23500 行＝T552 指令面板後 Python 數；測試 `test_fixde.js`、實測 PASS 6,186、exit 0）。零執行期相依、無框架無 CDN、全部美術程序化生成。
...
| ├ 地形／道路／覆蓋層 | 2963 | `/* ---------- 草地 4 變化 ---------- */` |
| ├ `PARTS` / `DRAFTS` / `mkBld` | 2798 | `const PARTS=` |
| ├ 服務建築與變體 | 3127 | `/* ---------- 公園 3 變化 ---------- */` |
|　├ 體育場| 3551 | `/* ---------- 體育場（T22；首個 2×2 多格建築；136×150、錨點 68/` |
|　├ 墓園| 4035 | `/* ---------- 墓園（T38；72×112 小教堂＋墓碑群＋柏樹＋夜間微弱燭光；3 ` |
|　├ T382：鐘樓細化| 4500 | `/* T382：鐘樓細化（零亂數）——門、台階、壁燈、石紋 */` |
|　├ T382：度假酒店細化| 4982 | `/* T382：度假酒店細化（零亂數）——陽台欄、大堂門廊、池邊躺椅、遮陽傘、水線、翼門、夜池邊` |
|　├ T382：社區菜園細化| 5474 | `/* T382：社區菜園細化（零亂數）——澆水壺、圍欄、小徑、補夜層（退修：澆水壺下移，避免 v` |
| ├ 多格地標長列 | 5653 | `T355 版面重排` |
| ├ FIX-B 尾端區起點 | 6019 | `/* ===== FIX-B 亂數流對齊：以下 T29/T30/T35/T36` |
|　├ T382：綠蔭停車 v2 細化| 6404 | `/* T382：綠蔭停車 v2 細化（零亂數）——充電樁、路緣、地磚 */` |
|　├ 公園新變體 3 種| 6873 | `/* ---------- 公園新變體 3 種（噴泉廣場／玫瑰園／遊樂場） ----------` |
|　├ 立面件：沿左右牆挑 dx 段落| 7365 | `// ---- 立面件：沿左右牆挑 dx 段落，y 落在該 dx 列牆面安全範圍內（yF-h+p` |
|　├ 街道設施小件| 7828 | `// ---- 街道設施小件（沿用 SPR.rdec/SPR.busStop 同款 64×40／` |
|　├ T150 變體大擴充：住宅/商業/工業每級 8→12 變體| 8085 | `/* ===== T150 變體大擴充：住宅/商業/工業每級 8→12 變體（k=1/2/3 各` |
|　├ T382：hero 後寫加蓋——救護車、入口雨棚、紅十字角燈 */| 8585 | `/* T382：hero 後寫加蓋——救護車、入口雨棚、紅十字角燈 */` |
|　├ T382：hero 後寫加蓋——候診椅、門廊、綠十字角燈 */| 9081 | `/* T382：hero 後寫加蓋——候診椅、門廊、綠十字角燈 */` |
|　├ T380：美術改畫在田外上緣自由區| 9572 | `/* T380：美術改畫在田外上緣自由區（y≤105；田心 cy=128 hh=20 不蓋），保` |
|　├ T278 RCI 屋頂雜項加蓋| 10036 | `/* ===== T278 RCI 屋頂雜項加蓋（TheoTown 級屋頂密度）：snowCap` |
| ├ 加蓋層 pass 群（T277→T345→**T382**） | 10095 | `const stampRoof=(key)=>{` / `/* ===== T382 全局美術極細化加蓋 pass` |
| └ T364 縮放管線與收尾 | 10527 | `const mkIndustry364=(kind)=>{` |
|　├ T425 統一底部貼合 pass| 11009 | `/* ===== T425 統一底部貼合 pass（buildSprites 絕對尾端）：T29` |
| §3 世界生成 `genWorld` / `newWorld` | 11296 | `FIX-K：天氣/災害殘留狀態種子化重置` |
| §4 `canPlace` / `placeCost` / `doPlace` | 11371 | `function canPlace(toolId,x,y){` |
|　├ doPlace| 11664 | `function doPlace(toolId,x,y,silent){` |
| `computePower` | 12376 | `function computePower(){` |
| `COVR` / `COV` / `covFieldOfK` / `stampCov` | 12590 | `const covFieldOfKBase=covFieldOfK;` |
| POL / NOISE 場 | 12611 | `function stampPolSrc(x,y,k,sign){` |
| LAND 場 | 12719 | `function landStaticAt(x,y){` |
| EDU / `rebuildCov` / `judgeWealth` | 12767 | `function rebuildCov(){` |
| `computeWater` / `resilience364At` | 12897 | `function computeWater(){` |
| §5 `aiStep()` AI 市長 | 13049 | `let acts=0;const MAXA=14,RESERVE=poor?50:350;` |
| `buildTickIndex()` | 13619 | `function buildTickIndex(){` |
| **`tick()` 全體** | 13632 | `rebuildNoise(tickBld); // T325` |
| ├ 第一經濟迴圈（計數／幸福） | 13727 | `for(const i88 of tickBld){` |
| ├ 產業鏈 | 13996 | `/* ===== T364b A 深加工鏈 BEGIN =====` |
| ├ 災害段 | 14058 | `// T69/T70 災害（可關）` |
| ├ 生長／升級／合併／火災 | 14226 | `// 生長：收集候選` |
| ├ **第二經濟迴圈（稅收守衛鏈）** | 14512 | `const civicMul=chN>0?1.03:1;` |
| └ 維護費／評分／`aiStep` 呼叫 | 14665 | `if(diff!==3)money+=income-upkeep;` |
|　├ updCars| 15107 | `function updCars(dt){` |
| §6 載具與煙／`computeCommute` | 15822 | `function computeCommute(){` |
| §7 繪製起點：`DIRSCR` / 雨雪 | 15392 | `const DIRSCR=[[4,-2],[4,2],[-4,2],[-4,-2]];` |
| 粒子池 `fxParts` | 15422 | `/* ---------- T159 粒子特效豐富化` |
| `daylight()` | 15518 | `function daylight(){` |
| `streetHash` | 15907 | `function streetHash(x,y,salt){` |
| **T367 旋轉變換層** | 15914 | `/* ===== T367 視角四向旋轉：view-space 變換層` |
| 地面快取全域 | 16003 | `let groundDirty=true,groundCache=null,` |
| **`draw()` 全體** | 16247 | `function draw(dt){` |
| ├ 地面層與 groundCache 重烘 | 16390 | `// ---- 地面層 ----（T96：離屏快取` |
| ├ `objs` 收集與深度排序 | 16809 | `// ---- 物件層（依深度排序） ----` |
| ├ 地格分支（建築十餘層加疊） | 17247 | `const t=o.t;` |
|　├ T403b/c 農事與牧場動態| 17730 | `/* ===== T403b/c 農事與牧場動態（T368a 純視覺路線：零狀態、零陣列，一切由` |
| ├ 天氣後製與晝夜 multiply | 17891 | `// ---- 天氣色調（雨天壓暗` |
| └ 夜燈層 `nightSprites` | 18086 | `// ---- 夜間燈光（T212` |
|　├ drawGroundMemory428| 18581 | `function drawGroundMemory428(g,x,y,sx,sy,z){` |
| `drawCursor` / `drawHoverLabel` / `toTile` | 18953 | `function drawHoverLabel(sxOf,syOf,z){` |
| undo 群組 helper | 19024 | `const openUndo=()=>{undoGroup={snaps:[],seen:{},spent:0};};` |
| `commitRect` 批次施作 | 19226 | `const hiBld=(x,y)=>{const b=T(idx(x,y)).bld;` |
| `UP_MAX` / `upCost` / `upgradeBld` | 19256 | `const UP_MAX={9:15,6:10,7:10,11:10,12:10` |
| `inspect()` 建築檢視面板 | 19282 | `function svcStatTable(x,y,b){` |
|　├ MOODS| 19713 | `const MOODS={` |
| §10 UI 起點：`buildToolbar` | 19980 | `function buildToolbar(){` |
| `drawMini` 小地圖 | 20089 | `const MINI_VIEW_NAME=` |
| `statTab` / `dataTable` / `serviceRefRows` | 20166 | `T319 標準化數據表格產生器` |
| `toast` / `updHud` / HUD 三鈕 | 20251 | `T311 提示訊息不刷屏` |
| 存檔槽面板／分享碼 | 20346 | `function showSlots(){` |
| 地圖／場景編輯器 | 20463 | `function showEditor(){` |
| 歷史曲線／城市顧問／因果追溯 | 20596 | `function cityAdvisor(allT522){` |
| `showStats()` 統計面板 | 21119 | `function showStats()` |
| 通知中心／成就 | 21590 | `T114：通知中心——HUD 鈴鐺按鈕` |
| `chipPanel()` 六晶片明細 | 21666 | `function chipPanel(which){` |
| `showHelp()` 五分頁指南 | 21868 | `const tabs=['🚦 上手流程'` |
| `undo()` / 快捷鍵 / 新手提示 | 22240 | `function checkHints(plants,roads){` |
| §11 存檔：槽位／RLE／`save` | 22272 | `function slotKey(n){return SAVEKEY+'.s'+n;}` |
| `MSZ` 多格尺寸反查表 | 22419 | `const MSZ={19:4,20:2,22:2,23:2,24:2,25:2` |
| `load()` | 22462 | `const vlen=saveShapeOk533;` |
| 自動存檔 | 22605 | `setInterval(()=>{if(tiles)save();},25000);` |
| 主迴圈 `advance` / `frame` | 22618 | `while(simAcc>=DAYLEN&&steps<8){` |
| `begin` / `toMainMenu` / 開始畫面注入 | 22760 | `function toMainMenu(){` |
| `window.GV` 除錯鉤子（194 鍵） | 22877 | `window.GV={` |
