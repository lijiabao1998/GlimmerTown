...
**現況（測繪基準）**：單檔 `index.html`，v11.51，約 20,539 行（實測檔案 20,539 行＝T425 footprint 根治後 Python 數
；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T425**（footprint 溢出根治：k65 重排收窄＋T345 色環跨線全城歸零＋尾部統一貼合）（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 4,408、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
