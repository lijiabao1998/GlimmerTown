...
**現況（測繪基準）**：單檔 `index.html`，v11.47，約 20,406 行（實測檔案 20,406 行＝T421 施工後 Python 數
；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T421**（ART-LOOP 第三特許 R01–R15：28 鍵上線、R05 兩鍵決定不動）（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 4,324、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
