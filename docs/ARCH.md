...
| LAND 場 | 12600 | `function landStaticAt(x,y){` |
| EDU / `rebuildCov` / `judgeWealth` | 12648 | `function rebuildCov(){` |
| `computeWater` / `resilience364At` | 12778 | `function computeWater(){` |
| §5 `aiStep()` AI 市長 | 12928 | `let acts=0;const MAXA=14,RESERVE=poor?50:350;` |
| `buildTickIndex()` | 13464 | `function buildTickIndex(){` |
| **`tick()` 全體** | 13477 | `rebuildNoise(tickBld); // T325` |
| ├ 第一經濟迴圈（計數／幸福） | 13572 | `for(const i88 of tickBld){` |
| ├ 產業鏈 | 13841 | `/* ===== T364b A 深加工鏈 BEGIN =====` |
| ├ 災害段 | 13898 | `// T69/T70 災害（可關）` |
| ├ 生長／升級／合併／火災 | 14066 | `// 生長：收集候選` |
| ├ **第二經濟迴圈（稅收守衛鏈）** | 14352 | `const civicMul=chN>0?1.03:1;` |
| └ 維護費／評分／`aiStep` 呼叫 | 14504 | `if(diff!==3)money+=income-upkeep;` |
|　├ updCars| 14946 | `function updCars(dt){` |
| §6 載具與煙／`computeCommute` | 15661 | `function computeCommute(){` |
| §7 繪製起點：`DIRSCR` / 雨雪 | 15231 | `const DIRSCR=[[4,-2],[4,2],[-4,2],[-4,-2]];` |
| 粒子池 `fxParts` | 15261 | `/* ---------- T159 粒子特效豐富化` |
| `daylight()` | 15357 | `function daylight(){` |
| `streetHash` | 15746 | `function streetHash(x,y,salt){` |
| **T367 旋轉變換層** | 15753 | `/* ===== T367 視角四向旋轉：view-space 變換層` |
| 地面快取全域 | 15842 | `let groundDirty=true,groundCache=null,` |
| **`draw()` 全體** | 16086 | `function draw(dt){` |
| ├ 地面層與 groundCache 重烘 | 16229 | `// ---- 地面層 ----（T96：離屏快取` |
| ├ `objs` 收集與深度排序 | 16648 | `// ---- 物件層（依深度排序） ----` |
| ├ 地格分支（建築十餘層加疊） | 17086 | `const t=o.t;` |
|　├ T403b/c 農事與牧場動態| 17569 | `/* ===== T403b/c 農事與牧場動態（T368a 純視覺路線：零狀態、零陣列，一切由` |
| ├ 天氣後製與晝夜 multiply | 17730 | `// ---- 天氣色調（雨天壓暗` |
| └ 夜燈層 `nightSprites` | 17925 | `// ---- 夜間燈光（T212` |
|　├ drawGroundMemory428| 18420 | `function drawGroundMemory428(g,x,y,sx,sy,z){` |
| `drawCursor` / `drawHoverLabel` / `toTile` | 18792 | `function drawHoverLabel(sxOf,syOf,z){` |
| undo 群組 helper | 18863 | `const openUndo=()=>{undoGroup={snaps:[],seen:{},spent:0};};` |
| `commitRect` 批次施作 | 19065 | `const hiBld=(x,y)=>{const b=T(idx(x,y)).bld;` |
| `UP_MAX` / `upCost` / `upgradeBld` | 19095 | `const UP_MAX={9:15,6:10,7:10,11:10,12:10` |
| `inspect()` 建築檢視面板 | 19121 | `function svcStatTable(x,y,b){` |
|　├ MOODS| 19552 | `const MOODS={` |
| §10 UI 起點：`buildToolbar` | 19819 | `function buildToolbar(){` |
| `drawMini` 小地圖 | 19928 | `const MINI_VIEW_NAME=` |
