...
**現況（測繪基準）**：單檔 `index.html`，v11.13，約 19,188 行（實測檔案 19,188 行＝T382 總覆核退修後重測繪；**行號會漂移，定位一律以 grep 錨點為準**——「實測檔案 N 行」這個措辭是 T371b 守衛的錨點字串，改寫前先看 test_fixde.js）；卡號已到 **T387a**（CHANGELOG 380+ 條）；測試 `test_fixde.js`、實測 PASS 3,000+、exit 0。零執行期相依、無框架無 CDN、全部美術程序化生成。
...
| 2 | 12713 `horizonY=Math.max(0,Math.round(syOf(0,0)+6*z))` | rot=1/3 地平線掉到菱形腰線、rot=2 掉到底點，T256 懸浮感復發（→ T378） |
| 3 | 12749 懸崖 `x===N-1\|\|y===N-1` | rot=2 時崖壁畫在後緣、近端無崖（→ T378） |
