...
> **版本線**：2026-08-08 起迭代分兩條線（桌面線＝本倉庫；移動線由 T420 衍生、在倉庫外維護並定期回來清查）。分岔點 tag `fork/mobile-T420`；協定與清查五條見 **`docs/LINEAGE.md`**。本文件描述的流程屬**桌面線**。
...
### 覆核注入的判紅必須看 exit code，不得只 grep FAIL 行（T343c 事故）

覆核者做移除注入時，用「輸出裡有沒有 FAIL 開頭的行」判紅綠。但測試檔**頂層**的守衛
（如 probe 錨點失配 `throw`）造成的是**未捕捉例外崩潰**——exit 1、stderr 是堆疊、
一行 FAIL 都沒有。覆核工具把四個真紅全誤讀成綠，據此發出錯誤退修，
之後又把施工者正確的事實校正反裁決了一次。以 exit code 重放才翻案。

**規則：注入重放的判定必須同時看 ①exit code ②FAIL 行 ③stderr 的 Error。**
`verify.py` 的套件檢查（exit=0＋完成標記＋0 FAIL 三者齊備才綠）早已編碼這個教訓——
覆核者的臨時腳本不得繞過它；能直接用 `python tools/verify.py` 判紅綠就不要自寫判定。
